...
module rec Version : sig
  ...
  let rec to_binary {major; minor; patch; suffix} =
  ...
  let rec to_text {major; minor; patch; suffix} =
    let _o = Runtime.Byte_output.create () in
...
and Code_generator_request : sig
  ...
  let rec to_binary {file_to_generate; parameter; proto_file; compiler_version} =
    let _o = Runtime.Byte_output.create () in
    ...
    Bin'.serialize_repeated_message_field
      15
      Google_protobuf_descriptor_pc.File_descriptor_proto.to_binary
      proto_file
      _o
    >>= fun () ->
  ...
  let rec of_binary input' =
    ...
    Bin'.decode_repeated_message_field
      15
      Google_protobuf_descriptor_pc.File_descriptor_proto.of_binary
      _m
    >>= fun proto_file ->
  ...
  let rec to_text {file_to_generate; parameter; proto_file; compiler_version} =
    let _o = Runtime.Byte_output.create () in
    Text'.serialize_repeated_field "file_to_generate" Field'.String_t file_to_generate _o
    >>= fun () ->
    Text'.serialize_optional_field "parameter" Field'.String_t parameter _o >>= fun () ->
    Text'.serialize_repeated_message_field
      "proto_file"
      Google_protobuf_descriptor_pc.File_descriptor_proto.to_text
      proto_file
      _o
    >>= fun () ->
    Text'.serialize_message_field "compiler_version" Version.to_text compiler_version _o
    >>= fun () -> Ok (Runtime.Byte_output.contents _o)

  let rec of_text input' =
    Ok (Runtime.Byte_input.create input') >>= Text'.deserialize_message >>= fun _m ->
    Text'.decode_repeated_field "file_to_generate" Field'.String_t _m
    >>= fun file_to_generate ->
    Text'.decode_optional_field "parameter" Field'.String_t _m >>= fun parameter ->
    Text'.decode_repeated_message_field
      "proto_file"
      Google_protobuf_descriptor_pc.File_descriptor_proto.of_text
      _m
    >>= fun proto_file ->
    Text'.decode_message_field "compiler_version" Version.of_text _m
    >>= fun compiler_version ->
    Ok {file_to_generate; parameter; proto_file; compiler_version}
end
...
and Code_generator_response : sig
  ...
  module rec File : sig
    ...
    let rec to_binary {name; insertion_point; content} =
      let _o = Runtime.Byte_output.create () in
    ...
    let rec to_text {name; insertion_point; content} =
      let _o = Runtime.Byte_output.create () in
      Text'.serialize_optional_field "name" Field'.String_t name _o >>= fun () ->
      Text'.serialize_optional_field "insertion_point" Field'.String_t insertion_point _o
      >>= fun () ->
    ...
    let rec of_text input' =
      ...
      Text'.decode_optional_field "insertion_point" Field'.String_t _m
      >>= fun insertion_point ->
  ...
  let rec to_binary {error; file} =
    let _o = Runtime.Byte_output.create () in
    Bin'.serialize_optional_field 1 Field'.String_t error _o >>= fun () ->
    Bin'.serialize_repeated_message_field
      15
      Code_generator_response.File.to_binary
      file
      _o
    >>= fun () -> Ok (Runtime.Byte_output.contents _o)
  ...
  let rec of_binary input' =
    ...
    Bin'.decode_repeated_message_field 15 Code_generator_response.File.of_binary _m
    >>= fun file -> Ok {error; file}

  let rec to_text {error; file} =
    let _o = Runtime.Byte_output.create () in
    Text'.serialize_optional_field "error" Field'.String_t error _o >>= fun () ->
    Text'.serialize_repeated_message_field
      "file"
      Code_generator_response.File.to_text
      file
      _o
    >>= fun () -> Ok (Runtime.Byte_output.contents _o)
  ...
  let rec of_text input' =
    ...
    Text'.decode_repeated_message_field "file" Code_generator_response.File.of_text _m
    >>= fun file -> Ok {error; file}
end
