...
let invariant_suite (type t) (module T : Utils.Serdes_testable with type t = t)
    protobuf_type_name values_to_test
  =
  Utils.invariant_suite
    (module T)
    ~protobuf_file_name:"types.proto"
    ~protobuf_type_name
    values_to_test

module Strings = struct
  let tests =
    invariant_suite (module P.String') "String"
    @@ List.map ["just a string"] ~f:(fun field -> P.String'.{field})
end

module Bytes = struct
  let tests =
    invariant_suite (module P.Bytes) "Bytes"
...
module Empty = struct
  let tests = invariant_suite (module P.Empty) "Empty" [()]
end
